...
// XXX: Document that while N: usize because that's what arrays in bitvec need,
// it is in reality always u16 and conversions are done at the interface of
// World and WorldInner
#[derive(Clone)]
...
impl<const N: usize> WorldInnerConst<N> {
    pub fn new(dims: [u16; 3], adjacency_rules: Vec<AdjacencyRule>, features: Features) -> Self {
        assert!(dims[0] > 0);
        assert!(dims[1] > 0);
        assert!(dims[2] > 0);
        assert!(!adjacency_rules.is_empty());
