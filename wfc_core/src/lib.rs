...
impl World {
    ...
    pub fn slot_module(&self, pos: [u16; 3], module: NonZeroU32) -> bool {
        let index = position_to_index(self.slots.len(), self.dims, pos);
        let slot = &self.slots[index];

        slot.contains(module.get())
    }

    pub fn set_slot_module(&mut self, pos: [u16; 3], module: NonZeroU32, value: bool) {
        let index = position_to_index(self.slots.len(), self.dims, pos);
        ...
        if value {
            ...
            slot.remove(module.get());
        }

        let slot_len = count_ones(&slot);
        let slot_contains_void = slot.contains(0);
        if slot_contains_void {
            assert_eq!(
                slot_len, 1,
                "If a slot contains the void module, it can only contain the void module",
            );
        }
    }
    ...
    pub fn set_slot_modules(&mut self, pos: [u16; 3], value: bool) {
        ...
        for module in 1..=cast_u32(self.module_count) {
            if value {
                slot.add(module);
            } else {
                slot.remove(module);
    ...
    pub fn slot_void(&self, pos: [u16; 3]) -> bool {
        let index = position_to_index(self.slots.len(), self.dims, pos);
        let slot = &self.slots[index];
    ...
    pub fn set_slot_void(&mut self, pos: [u16; 3], value: bool) {
        let index = position_to_index(self.slots.len(), self.dims, pos);
        let slot = &mut self.slots[index];
    ...
    /// Iterates over all the modules in a slot. 0 means the slot is void (and
    /// no other module) is set.
